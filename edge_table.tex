...
\addlinespace
Climate $\to$ HEE & Regional climate shapes domestic heat demand profiles and thereby the marginal value of efficiency measures: longitudinal heat demand monitoring of UK social housing shows substantial spatial variation in heating demand reflecting regional weather conditions \citep{aragon_influence_2022}. Because the absolute energy and cost savings delivered by a given fabric measure scale with underlying heat demand, this variation alters the cost-effectiveness and geographic targeting of retrofit, and hence the efficiency state ultimately achieved across the stock. \\
\end{longtable}
...
% =====================================================
% Plausible bidirectional edges / orientation sensitivity
% =====================================================

Table~\ref{tab:excluded-edges} above lists edges that are genuinely absent from the
DAG. Several other edges are present and committed to a single orientation but
describe relationships that are plausibly bidirectional; for these the question is not
whether to draw the edge but whether its orientation affects identification.
Table~\ref{tab:bidirectional-edges} reports a joint orientation-sensitivity analysis of
the total effect of HEE on MH over all $3^{8}=6561$ configurations in which each of
eight candidate edges is held at its committed orientation, reversed, or replaced by a
latent common cause (a bidirected $\leftrightarrow$ edge). The committed (all-original)
configuration returns the compact minimal adjustment set $\mathcal{A}=\{$Climate,
Economic\_Capital, Housing\_Characteristics, Tenure, Urbanicity$\}$, vindicating the
DAG as drawn: of the 6561 configurations, $73.7\%$ are cyclic, $18.9\%$ are
identifiable, and only $7.4\%$ are non-identifiable. Non-identifiability has a single
gateway --- a latent HEE\,$\leftrightarrow$\,Economic\_Capital common cause is present
in all 486 of 486 non-identifiable configurations --- with exactly two minimal
(Hamming-distance-2) breakers: HEE\,$\leftrightarrow$\,EC paired with
EC\,$\leftrightarrow$\,MH, and HEE\,$\leftrightarrow$\,EC paired with
EC\,$\leftrightarrow$\,PH. No outcome-side reorientation breaks identification. Because
reversal is itself cyclic for half the candidates, the substantively meaningful
robustness operation is the latent common cause ($A\leftrightarrow B$), not
arrow-flipping. Three entries previously listed as ``excluded'' reverse directions
(HEE\,$\to$\,Economic\_Capital, Economic\_Capital\,$\to$\,Education, and MH\,$\to$\,PH)
are superseded by this analysis and have been moved here, with their original rationale
preserved in the Resolution column.

\begin{longtable}{@{}p{1.9cm}p{1.9cm}p{1.4cm}p{2.7cm}p{4.9cm}@{}}
\caption{Plausible bidirectional edges and the joint orientation-sensitivity verdict for the total effect of HEE on MH. The DAG topology is unchanged; all eight committed orientations are retained. Source: full $3^{8}$ enumeration, \texttt{bidirectionality\_sensitivity.md}.}
\label{tab:bidirectional-edges}\\
\toprule
\textbf{Edge} & \textbf{Committed orientation} & \textbf{Class} & \textbf{Effect on identification} & \textbf{Resolution} \\
\midrule
\endfirsthead
\multicolumn{5}{l}{\textit{Table \ref{tab:bidirectional-edges} continued from previous page}}\\
\toprule
\textbf{Edge} & \textbf{Committed orientation} & \textbf{Class} & \textbf{Effect on identification} & \textbf{Resolution} \\
\midrule
\endhead
\bottomrule
\endfoot
HEE --- Economic\_Capital & Economic\_Capital $\to$ HEE & Confounder-side & Most load-bearing edge and the \emph{sole} gateway to non-identifiability: a latent HEE $\leftrightarrow$ Economic\_Capital common cause is present in all 486 of 486 non-identifiable configurations and forms both minimal breakers when paired with a latent EC--outcome leg. & Resolved by temporal ordering. Economic Capital is a baseline / pre-treatment confounder, measured before the HEE exposure contrast, which licenses the acyclic \textit{Economic\_Capital $\to$ HEE} encoding; the latent variant corresponds to the case where that temporal separation fails. The reverse \textit{HEE $\to$ Economic\_Capital} edge is not drawn: energy efficiency improvements reduce household energy bills, which at the household level represents an increase in disposable income. Including this edge would make HEE a cause of downstream economic capital and would substantially complicate identification. At the LSOA area level, however, the bill-saving effect of HEE is already represented within the DAG through the affordability channel to mental health (\textit{HEE $\to$ IEQ} captures thermal comfort; the financial stress relief operating through reduced fuel poverty is captured in the HEE $\to$ MH mechanism documented in \citet{willand_towards_2015} and \citet{grey_short-term_2017}). Adding a direct \textit{HEE $\to$ Economic\_Capital} edge would conflate the exposure of interest with one of its downstream consequences, rendering the target estimand ambiguous. The edge is omitted, and the affordability mechanism is acknowledged as part of the HEE--MH pathway rather than as a modifier of the confounder set. \\
\addlinespace
Economic\_Capital --- Education & Education $\to$ Economic\_Capital & Confounder-side & Inert in isolation but jointly load-bearing: pivotal in 210 of the joint configurations, though it does not alter $\mathcal{A}$ in the committed configuration. & Resolved by temporal ordering (baseline measurement). Education and Economic Capital are ascertained at baseline, before the HEE exposure contrast, so the acyclic \textit{Education $\to$ Economic\_Capital} direction encodes a feedback system observed at fixed times. The reverse \textit{Economic\_Capital $\to$ Education} edge is not drawn: parental economic resources shape children's educational opportunities through school quality, tutoring, and sustained enrolment; this intergenerational mechanism is well-documented \citep{blanden_educational_2013}. Including this edge would, however, introduce a cycle with the existing \textit{Education $\to$ Economic\_Capital} edge, which captures the adult labour-market returns to schooling. DAG formalism requires a directional choice. The \textit{Education $\to$ Economic\_Capital} direction is retained because (i) at any single cross-section at LSOA level, the dominant data-generating process for working-age earnings is the labour-market-returns mechanism; (ii) the intergenerational reverse-direction mechanism operates on timescales outside the temporal scope of the present analysis; and (iii) the latent node \textit{U\_AreaHistory} already absorbs part of the area-level intergenerational persistence that would otherwise require a \textit{Economic\_Capital $\to$ Education} edge \citep{lloyd_50-year_2024}. The bidirectional nature of the underlying relationship is an acknowledged limitation rather than a modelling error. \\
\addlinespace
Economic\_Capital --- Policy\_Scheme\_Access & Economic\_Capital $\to$ Policy\_Scheme\_Access & Confounder-side & Load-bearing: reversal to \textit{Policy\_Scheme\_Access $\to$ Economic\_Capital} forces Policy\_Scheme\_Access into the adjustment set. & Resolved by defended orientation plus a bounded failure mode. The committed \textit{Economic\_Capital $\to$ Policy\_Scheme\_Access} orientation is defended because ECO, GBIS and the Warm Home Discount are means-tested and area-targeted, so area economic capital determines scheme access. Under \textit{Economic\_Capital $\to$ Policy\_Scheme\_Access}, Policy\_Scheme\_Access is a descendant of Economic\_Capital and is not required in $\mathcal{A}$. Under the reversed orientation \textit{Policy\_Scheme\_Access $\to$ Economic\_Capital} it would instead be a required but \emph{unmeasured} confounder --- stated here as a named, bounded failure mode rather than silently dropped. \\
\addlinespace
Economic\_Capital --- MH & Economic\_Capital $\to$ MH & Confounder-side & A latent Economic\_Capital $\leftrightarrow$ MH common cause is one of the two minimal (Hamming-2) non-identifiability legs, in conjunction with a latent HEE $\leftrightarrow$ Economic\_Capital link. & Committed \textit{Economic\_Capital $\to$ MH} retained; Economic Capital is measured and adjusted within $\mathcal{A}$. Non-identifiability arises only if this association is \emph{additionally} treated as an unmeasured common cause jointly with a latent HEE $\leftrightarrow$ Economic\_Capital link --- i.e. only when Economic Capital is given an unmeasured common cause with both the exposure and the outcome. \\
\addlinespace
PH --- MH & PH $\to$ MH & Outcome-side & Indifferent for identification: neither the reported set $\mathcal{A}$ nor identifiability changes under reversal or a latent variant; only non-reported alternative minimal sets shift. Listed for transparency. & Committed \textit{PH $\to$ MH} retained. Common mental disorders increase physical morbidity through neuroendocrine, inflammatory, and behavioural pathways; mediation analyses using UK data find significant effects in both the physical-to-mental and mental-to-physical directions \citep{ohrnberger_relationship_2017, berk_comorbidity_2023}. The relationship is genuinely bidirectional, but the DAG framework requires a single directional choice. We retain \textit{PH $\to$ MH} and omit \textit{MH $\to$ PH} because, for the target estimand (the effect of HEE on MH), the \textit{PH $\to$ MH} direction is what creates backdoor paths through physical health and therefore what matters for confounder identification. Prioritising the direction that threatens identification is the more conservative modelling choice; the reverse pathway, while real, would introduce a cycle without altering the adjustment set for our exposure-outcome pair. The implicit feedback loop is an acknowledged limitation of a cross-sectional DAG applied to a substantively dynamic system. \\
\addlinespace
Substance\_Abuse --- MH & Substance\_Abuse $\to$ MH & Outcome-side & Fully robust: never pivotal among valid graphs (0 of 2187 contexts). Orientation never changes any adjustment set; reversal only ever induces a cycle. Listed for transparency. & Committed \textit{Substance\_Abuse $\to$ MH} retained; its orientation is immaterial to identification of the HEE--MH effect, and the only consequence of reversal is an invalid (cyclic) graph. \\
\addlinespace
Isolation --- MH & Isolation $\to$ MH & Outcome-side & Indifferent for identification: the reported set $\mathcal{A}$ and identifiability are unaffected; only non-reported alternative minimal sets shift. Listed for transparency. & Committed \textit{Isolation $\to$ MH} retained; the orientation does not affect the reported adjustment set $\mathcal{A}$ or identifiability of the HEE--MH effect. \\
\addlinespace
Economic\_Capital --- PH & Economic\_Capital $\to$ PH & Confounder-side & A latent Economic\_Capital $\leftrightarrow$ PH common cause is the other minimal (Hamming-2) non-identifiability leg, in conjunction with a latent HEE $\leftrightarrow$ Economic\_Capital link. & Committed \textit{Economic\_Capital $\to$ PH} retained; both Economic Capital and PH are handled within the graph. Non-identifiability arises only if this association is \emph{additionally} treated as an unmeasured common cause jointly with a latent HEE $\leftrightarrow$ Economic\_Capital link. \\
\end{longtable}